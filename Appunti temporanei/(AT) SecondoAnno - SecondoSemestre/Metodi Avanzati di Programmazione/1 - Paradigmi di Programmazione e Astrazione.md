...
- **Non-side-effecting**: procedono creando continuamente nuovi dati. Questi paradigmi includono quelli che tradizionalmente sono detti **funzionali** (si da ad una funzione dei valori e lei produce un risultato, non modificando valori esistenti ma calcolandone altri e creandone copie eventuali). Una volta allocato in memoria, un dato non può più essere modificato. Una "funzione pura" prende in input dei valori e ne restituisce di nuovi, senza mai intaccare gli originali né alterare lo stato esterno.
...
Un modulo che ha in input un’espressione e un comando, iterando l’esecuzione del comando fintanto che l’espressione è vera è un modulo che implementa il costrutto di controllo **while()**. 
Sappiamo benissimo che l'implementazione del while è un implementazione **nascosta**.
...
## ATTENZIONE
Alcuni esempi sono stati omessi poichè potrebbero risultare troppo prolissi, nel caso li si vogliano visionare si possono trovare nel file seguente, estratto dalle slide del prof:
[[1.1 - (Slide Esempi) Paradigmi di programmazione e astrazione.pdf]]